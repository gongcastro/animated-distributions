...
anim = @animate for i=indices
    ...
    annotate!(median(x), 0.3-0.3/4, text(string.("σ = ", σ[i]), :black, :center, 12))
    annotate!(maximum(x), 0.3-0.3/20, text("@gongcastro", :black, :right, 6))
    p2 = plot(
        x, y[:, :, i, 2],
...
gif(anim, "Figures/cauchy.gif", fps=30)
